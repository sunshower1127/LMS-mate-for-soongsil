...
while True :
    ...
    for subject_btn in subject_btns :
        ...
        titles = [video_icon.parent().find(tag='a').text()
    ...
    ''' 강의영상 보기 '''
    for title in titles :
        web.iframe('default', 'tool_content')
        ...
        current_time = web.find(text='학습 진행 상태').parent().find(n=2).text().split('(')[0]

        time_left = (get_datetime(total_duration) - get_datetime(current_time)).total_seconds()
